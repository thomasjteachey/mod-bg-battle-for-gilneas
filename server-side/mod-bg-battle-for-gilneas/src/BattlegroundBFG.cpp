...
void BattlegroundBFG::StartingEventOpenDoors()
{
    ...
    _bgEvents.ScheduleEvent(BG_BFG_EVENT_ALLIANCE_TICK, 3000ms);
    _bgEvents.ScheduleEvent(BG_BFG_EVENT_HORDE_TICK, 3000ms);
}
...
void BattlegroundBFG::FillInitialWorldStates(WorldPackets::WorldState::InitWorldStates& pkt)
{
    auto add = [&](uint32 id, uint32 val)
    {
        pkt.Worldstates.emplace_back(id, val);
    };

    for (uint8 node = 0; node < GILNEAS_BG_DYNAMIC_NODES_COUNT; ++node)
    {
        if (_capturePointInfo[node]._state == GILNEAS_BG_NODE_TYPE_NEUTRAL)
            add(_capturePointInfo[node]._iconNone, 1);

        for (uint8 i = GILNEAS_BG_NODE_STATUS_ALLY_OCCUPIED; i <= GILNEAS_BG_NODE_STATUS_HORDE_CONTESTED; ++i)
            add(_capturePointInfo[node]._iconCapture + i - 1, _capturePointInfo[node]._state == i ? 1u : 0u);
    }

    add(GILNEAS_BG_OP_OCCUPIED_BASES_ALLY, _controlledPoints[TEAM_ALLIANCE]);
    add(GILNEAS_BG_OP_OCCUPIED_BASES_HORDE, _controlledPoints[TEAM_HORDE]);
    add(GILNEAS_BG_OP_RESOURCES_MAX, GILNEAS_BG_MAX_TEAM_SCORE);
    add(GILNEAS_BG_OP_RESOURCES_WARNING, GILNEAS_BG_WARNING_NEAR_VICTORY_SCORE);
    add(GILNEAS_BG_OP_RESOURCES_ALLY, m_TeamScores[TEAM_ALLIANCE]);
    add(GILNEAS_BG_OP_RESOURCES_HORDE, m_TeamScores[TEAM_HORDE]);
    add(0x745, 0x2); // unknown worldstate carried forward from original
}
...
void AddBattleForGilneasScripts() {
    ...
    Player::bgZoneIdToFillWorldStates[5449] =
        [](Battleground* bg, WorldPackets::WorldState::InitWorldStates& pkt)
    {
        if (!bg || bg->GetBgTypeID(true) != BATTLEGROUND_BFG)
            return;

        bg->FillInitialWorldStates(pkt);
    };
}
